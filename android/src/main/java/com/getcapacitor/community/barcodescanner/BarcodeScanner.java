...
public class BarcodeScanner extends Plugin implements BarcodeCallback {
    ...
    public void takePhoto(PluginCall call) {
        if (mBarcodeView == null) {
            call.reject("Camera preview is not active");
            return;
        }

        getActivity()
            .runOnUiThread(() -> {
                mBarcodeView.pause(); // Pause barcode scanning temporarily

                // Take the picture from the camera preview
                mBarcodeView
                    .getBarcodeView()
                    .addFrameReceiver(frame -> {
